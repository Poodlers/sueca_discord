...
# Embed declarations
embed_next_player = Embed(
    title="It's now your turn!", description="Please play one of your cards!"
)
embed = Embed(title="Sueca", description="Bem vindos ao jogo da sueca!", color=0x2E6CBD)
embed.add_field(
    name="Please React to This Message with your corresponding team to Enter the Game!",
    value="We are waiting 30 seconds for people!",
)
embed.set_image(
    url="https://i1.wp.com/etili.co/wp-content/uploads/2018/09/sueca.jpg?fit=900%2C665"
)
embed_invalid = Embed(
    title="No people!", description="No one wants to play sueca! Sad!", color=0xD23B20
)
embed_shit = Embed(
    title="Too many people on one team!",
    description="Aborting the game!",
    color=0x2E6CBD,
)
embed_player = Embed(
    title="Welcome to the Game of Sueca!",
    description="Here's your starting cards!",
    color=0x2E6CBD,
)
embed_player.add_field(
    name="Use command !play <0 - 9> to play a card!",
    value="You can use command !leave to leave the game anytime!",
)
embed_initial = Embed(
    title="You are to make the first move!", description="Go!", color=0x2E6CBD
)
embed_played = Embed(title="Current State of The Game")
embed_green_round = Embed(
    title="Epic Green Win!", description="Green Takes The Round!", color=0x2E6CBD
)
embed_blue_round = Embed(
    title="Epic Blue Win!", description="Blue Takes The Round!", color=0x317ADF
)
embed_next_player = Embed(title="It's your turn now. Please Play one of your cards!")
embed_fill_bots = Embed(
    title="There are not enough humans!",
    description="Populating the game with bots!",
    color=0x2E6CBD,
)
embed_no_humans = Embed(
    title="The last human in the game has left!",
    description="Aborting the game!",
    color=0xD23B20,
)
...
class Player:
    def __init__(self, team, cards, user, isBot):
        ...
        if self.isBot:
            ...
            with requests.get(self.user.display_avatar.url) as r:
                img_data = r.content
...
async def continue_play(player_game):
    while True:
        if player_game.current == player_game.last_winner:  # the round has ended
            ...
            if check_if_game_over(player_game.players[player_game.current]):
                ...
                if player_game.points_green > player_game.points_blue:
                    embed_win_green = Embed(
                        title="It's an epic win for green team! Congrats guys!",
                        value="Green team has come out on top with {} points, while blue team scored only {} points!".format(
                            player_game.points_green, player_game.points_blue
                        ),
                    )
                    await player_game.channel.send(embed=embed_win_green)
                elif player_game.points_blue > player_game.points_green:
                    embed_blue_win = Embed(
                        title="It's an epic win for blue team! Yay! So much win!",
                        value="Blue team has come out on top with {} points, while green team scored only {} points!".format(
                            player_game.points_blue, player_game.points_green
                        ),
                    )
                    await player_game.channel.send(embed=embed_blue_win)
                else:
                    embed_draw = Embed(
                        title="It's a draw! What a shame!",
                        value="Both team score a total of {} points!".format(
                            player_game.points_blue
                        ),
                    )
        ...
        if player_game.players[player_game.current].isBot == False:
            await player_game.players[player_game.current].channel.send(
                "{}".format(player_game.players[player_game.current].user.mention)
            )
            await player_game.players[player_game.current].channel.send(
                embed=embed_next_player
            )
            image_path = create_hand_image(
                player_game.players[player_game.current].cards
            )
            await player_game.players[player_game.current].channel.send(
            ...
            return
        while (
            player_game.players[player_game.current].isBot
            and len(player_game.current_board) != 4
        ):
            play = bot_plays(
                player_game.players[player_game.current].cards,
                player_game.current_board,
                player_game.trump,
            )
...
async def sueca(ctx):
    ...
    for reaction in message.reactions:
        ...
        reaction_users.pop(0)
        for user in reaction_users:
            for player in players:
                if player.user == user:
                    await ctx.send("One of you selected more than one team! Dummy!")
                    return
    ...
    if len(players) < 4:
        ...
        for a in range(0, n_blues_bots):
            ...
            players.append(
                Player("blue", deck[0 + 10 * i : 10 + 10 * i], bot_name, True)
            )
        ...
        for a in range(0, n_greens_bots):
            ...
            del bot_names[bot_index]
            players.append(
                Player("green", deck[0 + 10 * i : 10 + 10 * i], bot_name, True)
            )
    ...
    embed_trump = Embed(
        title="The trump for this game will be...",
        description="{}".format(trump),
        color=0x2E6CBD,
    )
    ...
    for player in players:
        ...
        channel = await guild.create_text_channel(
            "{}'s sueca playroom".format(player.user.display_name),
    ...
    while new_game.players[new_game.current].isBot:
        play = bot_plays(
            new_game.players[new_game.current].cards,
    ...
    next_player = new_game.players[new_game.current]
    await next_player.channel.send("{}".format(next_player.user.mention))
    await next_player.channel.send(embed=embed_next_player)
...
async def leave(ctx):
    ...
    for i in range(0, len(player_game.players)):
        if (
            player_game.players[i].user == user
        ):  # replace the guy that's leaving with a bot
...
async def play(ctx, arg):
    ...
    try:
        ...
        return
    if (
        player_game.players[player_game.current].user == user
        and ctx.channel == player_game.players[player_game.current].channel
    ):
        try:
            arg = int(arg)
        except:
        ...
        if player_game.current_board != [] and is_illegal(
            play,
            player_game.current_board,
            player_game.players[player_game.current].cards,
        ):
...
def create_hand_image(cards):
    ...
    for card in cards:
        ...
        d.text(
            (acc_width + img_temp.width - 110, 10), str(i), font=fnt, fill=(0, 95, 255)
        )
...
def create_board_image(game):
    ...
    for player in game.players:
        if player.play != "":
            card = Image.open(
                "PNG/{}.png".format(player.play[0] + translation[player.play[1]])
        ...
        if player.isBot:
            ...
            names.append(player.user.display_name)
    ...
    dst.paste(
        images[2][0],
        (dst.width - avatar_width - 30, dst.height // 2 - avatar_height // 2),
    )
    dst.paste(
        rect_green,
        (dst.width - avatar_width - 50, dst.height // 2 - avatar_height // 2),
    )
    dst.paste(
        images[2][1],
        (
            dst.width - avatar_width - card_width - 70,
            dst.height // 2 - avatar_height // 2,
        ),
    )
    dst.paste(
        images[3][0],
        (dst.width // 2 - avatar_width - 150, dst.height - avatar_height - 10),
    )
    dst.paste(
        rect_blue,
        (dst.width // 2 - avatar_width - 170, dst.height - avatar_height - 10),
    )
    dst.paste(
        images[3][1], (dst.width // 2 - card_width // 2, dst.height - card_height - 10)
    )
    d = ImageDraw.Draw(dst)
    d.text(
        (10, 20),
        "Green Team Points:{}".format(game.points_green),
        fill=(0, 0, 0),
        font=fnt,
    )
    d.text(
        (dst.width - 580, dst.height - 60),
        "Blue Team Points:{}".format(game.points_blue),
        fill=(0, 0, 0),
        font=fnt,
    )
    d.text(
        (10, dst.height // 2 - avatar_height // 2 - 70),
        names[0],
        font=fnt,
        fill=(0, 0, 0),
    )
    d.text(
        (dst.width // 2 + avatar_width // 2 + 10, 10),
        names[1],
        font=fnt,
        fill=(0, 0, 0),
    )
    d.text(
        (dst.width - avatar_width - 40, dst.height // 2 - avatar_height // 2 - 75),
        names[2],
        font=fnt,
        fill=(0, 0, 0),
    )
    d.text(
        (dst.width // 2 - avatar_width - 150, dst.height - avatar_height - 80),
        names[3],
        font=fnt,
        fill=(0, 0, 0),
    )
...
def bot_plays(cards, current_board, trump):
    if current_board == []:
        ...
        if len(current_board) == 2:
            if card_wins(
                current_board[0], current_board[1], trump
            ):  # checks if the first card wins over the second
                return max_value
            elif (trump_card != "") and (
                cards_value[current_board[0][0]] > 1
                or cards_value[current_board[1][0]] > 1
            ):
                return trump_card
            else:
                return min_value
            ...
            if len(current_board) == 1:
                ...
                if card_wins(current_board[1], current_board[0], trump) and card_wins(
                    current_board[1], current_board[2], trump
                ):
                    return max_value
                for card in possible_plays:
                    if card_wins(card, current_board[0], trump) and card_wins(
                        card, current_board[2], trump
                    ):
                        winning_cards.append(card)
            if winning_cards == []:
                return min_value
                ...
                for card in winning_cards:
                    if (
                        card[1] != trump
                    ):  # if theres a winning_card that isnt a trump the bot will play it, otherwise play a trump
                        return card
